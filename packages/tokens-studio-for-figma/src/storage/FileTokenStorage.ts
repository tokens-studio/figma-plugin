    ...
    try {
      if (this.flags.multiFileEnabled && this.files.length > 1) {
        const jsonFiles = Array.from(this.files)
          .filter((file) => file.webkitRelativePath.endsWith('.json'))
          .sort((a, b) => (a.webkitRelativePath && b.webkitRelativePath ? a.webkitRelativePath.localeCompare(b.webkitRelativePath) : 0));
