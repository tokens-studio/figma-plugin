...
export default function ChangedTokenItem({ token }: { token: ImportToken }) {
  const { t } = useTranslation(['general']);

  return (
    <Stack direction="row" justify="between" css={{ padding: '$2 0' }}>
      <Stack
        direction={token.importType === 'REMOVE' ? 'row' : 'column'}
        gap={1}
        justify="between"
        css={{ width: '100%' }}
      >
        <Text bold size="small" css={{ wordBreak: 'break-word' }}>
          {token.name}
        </Text>
        ...
        {((token.importType === 'UPDATE' && token.oldDescription) ||
          (token.importType === 'NEW' && token.description)) && (
          <Stack direction="row" align="start" justify="between" gap={1}>
            ...
            <Stack direction="column" align="end" gap={1} css={{ maxWidth: '60%' }}>
              {token.oldDescription ? <StyledDiff type="danger">{token.oldDescription}</StyledDiff> : null}
              <StyledDiff type="success">{token.description}</StyledDiff>
            </Stack>
          </Stack>
        )}
        {token.importType === 'REMOVE' && <StyledDiff type="danger">{t('removed')}</StyledDiff>}
      </Stack>
    </Stack>
  );
}
