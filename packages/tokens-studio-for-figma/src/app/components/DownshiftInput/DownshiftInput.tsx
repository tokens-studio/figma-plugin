...
import { Box, Stack, Tooltip, Text, IconButton } from '@tokens-studio/ui';
...
export const DownshiftInput: React.FunctionComponent<
  React.PropsWithChildren<React.PropsWithChildren<DownShiftProps>>
> = ({
  name,
  type,
  label,
  ...
  const filteredTokenItems = useMemo(() => {
    ...
    if (isDocumentationType(type as Properties)) {
      initialFilteredValues = resolvedTokens
        .filter((token: SingleToken) => token.name.toLowerCase() !== initialName?.toLowerCase())
        .sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' }));
    } else {
      initialFilteredValues = resolvedTokens
        .filter(
          (token: SingleToken) =>
            referenceTokenTypes.includes(token?.type) && token.name.toLowerCase() !== initialName?.toLowerCase(),
        )
        .sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' }));
    }
    if (searchInput) {
      return initialFilteredValues.filter((token: SingleToken) =>
        fuzzySearch(searchInput.toLowerCase(), token.name.toLowerCase()),
      );
    }
  ...
  const filteredValues = useMemo(() => {
    ...
    if (currentSearchField === 'Weights' && externalFontFamily) {
      initialFilteredValues = figmaFonts
        .filter((font) => font.fontName.family === externalFontFamily)
        .map((selectedFont) => selectedFont.fontName.style);
    }
    if (searchInput) {
      ...
      {({ highlightedIndex, getItemProps, getInputProps }) => (
        <div style={{ position: 'relative' }}>
          ...
          <Box css={{ display: 'flex', position: 'relative', width: '100%' }} className="input">
            {!!inlineLabel && !prefix && (
              <Tooltip label={name}>
                <StyledPrefix isText css={{ height: 'auto' }}>
                  {label}
                </StyledPrefix>
              </Tooltip>
            )}
            {!!prefix && (
              <StyledPrefix isComposition={isComposition} css={{ height: 'auto' }}>
                {prefix}
              </StyledPrefix>
            )}
            <MentionsInput
              ...
              <Popover.Root open={showAutoSuggest} onOpenChange={setShowAutoSuggest}>
                ...
                <Popover.Portal>
                  <Popover.Content
                    ...
                    align="end"
                    sideOffset={4}
                    style={{ pointerEvents: 'all', width: 'var(--radix-popover-trigger-width)' }}
                  >
                    <Box
                      css={{
                    ...
                    >
                      ...
                      {currentSearchField === 'Tokens' && filteredTokenItems.length > 0 && (
                        <StyledList
                          ...
                          height={Math.min(downShiftContainerHeight, 30 * filteredTokenItems.length)}
                          width="100%"
                          itemCount={filteredTokenItems.length}
                          itemSize={30}
                        >
                          {({ index, style }) => {
                      ...
                      {currentSearchField !== 'Tokens' && filteredValues.length > 0 && (
                        <StyledList
                          className="content scroll-container"
                          height={Math.min(downShiftContainerHeight, 30 * filteredValues.length)}
                          width="100%"
                      ...
                      {((currentSearchField !== 'Tokens' && filteredValues.length === 0) ||
                        (currentSearchField === 'Tokens' && filteredTokenItems.length === 0)) && (
                        <Box css={{ padding: '$3', color: '$fgMuted', fontSize: '$small' }}>No suggestions found</Box>
                      )}
                    </Box>
                  </Popover.Content>
                </Popover.Portal>
