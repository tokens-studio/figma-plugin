...
export default function CompositionTokenForm({
  ...
  const propertiesMenu = React.useMemo(
    () => filterValidCompositionTokenTypes(Object.keys(Properties)).map((key: string) => String(Properties[key as CompositionTokenProperty])),
    [],
  );
  ...
  const handleReorder = React.useCallback(
    (reorderedItems: string[]) => {
      ...
      const rearrangedTokenValue = Object.assign(
        ...
        ...Object.keys(internalEditTokenValue)
          .sort(
            (a, b) => Number(newOrderObj[a as CompositionTokenProperty]) - Number(newOrderObj[b as CompositionTokenProperty]),
          )
          .map((x) => ({ [x as CompositionTokenProperty]: internalEditTokenValue[x as CompositionTokenProperty] })),
      );
      setOrderObj(newOrderObj);
