...
function ChangedStateList({ type }: { type: 'push' | 'pull' }) {
  ...
  const handleSetIntCollapsed = React.useCallback(
    (e: React.MouseEvent<HTMLButtonElement>, tokenSet: string) => {
      e.stopPropagation();
      if (e.altKey) {
        if (collapsed) {
  ...
  return (
    <Stack direction="column" gap={1} css={{ padding: '$4', width: '100%' }}>
      {Object.entries(changedState.tokens).length > 0
        && Object.entries(changedState.tokens)?.map(([tokenSet, tokenList]) => (
          <Box key={tokenSet} css={{ maxWidth: '100%' }}>
            <ChangeStateListingHeading
              count={tokenList.length}
              onCollapse={handleSetIntCollapsed}
              set={tokenSet}
              label={tokenSet}
              isCollapsed={collapsedChangedStateList.includes(tokenSet)}
            />
            {!collapsedChangedStateList.includes(tokenSet)
              && tokenList
              && tokenList.map((token) => <ChangedTokenItem key={token.name} token={token} />)}
          </Box>
        ))}
      {changedState.themes.length > 0 && (
        <>
          <ChangeStateListingHeading
            ...
            set="$themes"
            label="$themes"
            isCollapsed={collapsedChangedStateList.includes('$themes')}
          />
          {!collapsedChangedStateList.includes('$themes')
            && changedState.themes.map((theme) => (
              <Stack direction="row" justify="between" align="center" gap={3} css={{ padding: '$1 $4' }} key={theme.id}>
                <Text bold size="small">
                  {theme.name}
                </Text>
                {theme.importType === 'REMOVE' && <StyledDiff type="danger">{t('configurationRemoved')}</StyledDiff>}
      ...
      {changedState.metadata?.tokenSetOrder && Object.entries(changedState.metadata.tokenSetOrder).length > 0 && (
        <>
          <ChangeStateListingHeading
            ...
            set="$metadata"
            label="$metadata"
            isCollapsed={collapsedChangedStateList.includes('$metadata')}
          />
          {!collapsedChangedStateList.includes('$metadata') && (
            <Stack direction="row" justify="end" align="end" css={{ padding: '$1 $4' }}>
              <StyledDiff type="danger">{t('configurationChanged')}</StyledDiff>
            </Stack>
          )}
        </>
      )}
