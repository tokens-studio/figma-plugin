...
import { Box, IconButton, Stack, ToggleGroup } from '@tokens-studio/ui';
...
export default function SingleBoxShadowInput({
  ...
  const handleBoxshadowValueDownShiftInputChange = React.useCallback(
  ...
  const exchangeBoxshadowValue = React.useCallback(
    (newValue: TokenBoxshadowValue | TokenBoxshadowValue[]) => {
  ...
  const onMoveDebounce = React.useCallback(
      ...
      <Box css={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} ref={ref}>
        ...
        <Stack justify="end" css={{ width: '100%' }}>
          <ToggleGroup type="single" value={shadowItem?.type ?? newTokenValue.type} onValueChange={onTypeChange}>
            <ToggleGroup.Item iconOnly={false} value="innerShadow">
              {t('shadow.inner')}
            </ToggleGroup.Item>
            <ToggleGroup.Item iconOnly={false} value="dropShadow">
              {t('shadow.drop')}
            </ToggleGroup.Item>
          </ToggleGroup>
        </Stack>
        ...
        {Object.keys(propertyTypes).map((key, keyIndex) => (
          <>
            <SingleBoxShadowDownShiftInput
              ...
              value={String(
                shadowItem?.[key as keyof typeof propertyTypes] ?? newTokenValue[key as keyof typeof newTokenValue],
              )}
