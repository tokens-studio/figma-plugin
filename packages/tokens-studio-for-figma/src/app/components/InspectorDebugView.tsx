...
export default function InspectorDebugView({ resolvedTokens }: { resolvedTokens: SingleToken[] }) {
  ...
  const getResolvedValue = useCallback(
    (property: string, value: string) => {
      ...
      const resolvedValue = uiState.selectionValues.find((item) => item.category === property && item.value === value)
        ?.resolvedValue;
      if (resolvedValue) {
  ...
  function renderBlankslate() {
    if (uiState.selectedLayers > 1)
      return <EmptyState title={t('moreThan1Layer.title')} description={t('moreThan1Layer.description')} />;
    return (
      <EmptyState
        title={uiState.selectedLayers === 1 ? t('noTokensFound') : t('noLayersSelected')}
        description={
          uiState.selectedLayers === 1 ? t('selectedLayerContainsNoTokens') : t('selectALayerToSeeAppliedTokens')
        }
      />
    );
  }
            ...
            {Object.entries(uiState.mainNodeSelectionValues)
              ...
              .map(([property, value]) => (
                <StyledCode key={property}>
                  ...
                  <StyledInspectBadge>{typeof value === 'string' && value.split('.').join('-')}</StyledInspectBadge>
                  <Text size="xsmall" muted css={{ wordBreak: 'break-all' }}>{`/* ${getResolvedValue(
                    property,
                    value,
                  )} */`}</Text>
                </StyledCode>
