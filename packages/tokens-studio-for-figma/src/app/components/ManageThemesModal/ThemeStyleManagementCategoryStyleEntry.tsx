...
import { Box, IconButton, Tooltip, Checkbox } from '@tokens-studio/ui';
...
export const ThemeStyleManagementCategoryStyleEntry: React.FC<
  React.PropsWithChildren<React.PropsWithChildren<Props>>
> = ({ token, styleInfo, icon, isChecked, onDisconnectStyle, handleToggleSelectedStyle }) => {
    ...
    >
      ...
      >
        <Tooltip
          ...
          label={
            <Stack direction="column" align="start" gap={1} css={{ wordBreak: 'break-word' }}>
              ...
              <Text css={{ color: '$tooltipFgMuted' }}>{styleInfo.name}</Text>
            </Stack>
          }
        >
          <Box
          ...
          >
            ...
            {!styleInfo.name && !styleInfo.failedToResolve && <ResolvingLoader />}
            ...
            {styleInfo.name && (
              <Text bold size="small" title={styleInfo.name} css={{ overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {styleInfo.name}
              </Text>
            )}
          </Box>
        ...
        <IconButton
          size="small"
          variant="invisible"
          tooltip="Detach style"
          icon={<LinkBreak2Icon />}
          data-testid="themestylemanagementcategorystyleentry-unlink"
          onClick={handleDisconnectStyle}
        />
      </Flex>
    </Flex>
  );
