...
export function ThemeListItemContent({ item, isActive, groupName, onOpen }: Props) {
  ...
  return (
    ...
    >
      <StyledDragGrabber<ThemeObject> item={item} canReorder={!editProhibited} onDragStart={handleDragStart} />
      <SingleThemeEntry key={item.id} theme={item} isActive={isActive} groupName={groupName} onOpen={onOpen} />
    </StyledDragButton>
  );
}
