...
export const ThemeStyleManagementCategory: React.FC<React.PropsWithChildren<React.PropsWithChildren<Props>>> = ({
  ...
  const isAttachingLocalStyles = useSelector(
    useCallback(
      (state: RootState) => isWaitingForBackgroundJobSelector(state, BackgroundJobs.UI_ATTACHING_LOCAL_STYLES),
      [],
    ),
  );
  ...
  const handleSelectAll = React.useCallback(() => {
    setSelectedStyles(selectedStyles.length === stylesEntries.length ? [] : stylesEntries.map(([token]) => token));
  ...
  const handleToggleSelectedStyle = useCallback(
    (token: string) => {
      setSelectedStyles(
        selectedStyles.includes(token) ? selectedStyles.filter((style) => style !== token) : [...selectedStyles, token],
      );
    },
    [selectedStyles],
  );
  ...
  return (
    <Accordion
      ...
      extra={
        <Button size="small" disabled={isAttachingLocalStyles} onClick={onAttachLocalStyles}>
          {t('attachLocalStyles')}
        </Button>
      }
    ...
    >
      {stylesEntries.length > 0 && (
        ...
        >
          <Stack align="center" gap={3} css={{ fontSize: '$small' }}>
            <Checkbox
          ...
          <Stack gap={1}>
            <Button
              onClick={handleDisconnectSelectedStyles}
