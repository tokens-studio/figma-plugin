...
export const TokenSetThemeItem: React.FC<React.PropsWithChildren<React.PropsWithChildren<Props>>> = ({
  ...
  const tokenSetStatus = useMemo(() => value?.[item.path] ?? TokenSetStatus.DISABLED, [item.path, value]);
        ...
        <Stack direction="row" justify="between" align="center" css={{ width: '100%' }}>
          ...
          >
            {tokenSetStatusValues.map((status) => (
              <ToggleGroup.Item
                key={status}
                tooltip={t(tokenSetStatusLabels[status])}
                tooltipSide="top"
                value={status}
                data-testid={`tokensettheme-item--ToggleGroup-content--${item.label}--${status}`}
              >
                {statusIcon(status)}
              </ToggleGroup.Item>
            ))}
