...
export const ThemeSelector: React.FC<React.PropsWithChildren<React.PropsWithChildren<unknown>>> = () => {
  ...
  const groupNames = useMemo(
    () => [...new Set(availableThemes.map((theme) => theme.group || INTERNAL_THEMES_NO_GROUP))],
    [availableThemes],
  );
  ...
  const renderThemeOption = useCallback(
    (themes: AvailableTheme[]) => themes.map(({ label, value }) => {
          ...
          return (
            filteredThemes.length > 0 && (
              <DropdownMenu.RadioGroup
                key={groupName}
                value={typeof activeTheme[groupName] !== 'undefined' ? activeTheme[groupName] : ''}
              >
                <Text css={{ color: '$fgMuted', padding: '$2 $3' }}>
                  {groupName === INTERNAL_THEMES_NO_GROUP ? INTERNAL_THEMES_NO_GROUP_LABEL : groupName}
                </Text>
