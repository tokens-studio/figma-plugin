    ...
    <Tooltip label={`Type: ${token.type}`}>
      <Box
        css={{
          ...
          borderRadius: '$small',
          border:
            typeof token.value === 'string' && (token.value as string).startsWith('#')
              ? `1px solid ${token.value as string}`
              : undefined,
        }}
      >
        {isSingleBoxShadowToken(token) && Array.isArray(token.value) ? (
          ...
          <>
            ...
            {typeof token.value === 'string' ? (
              <Text>{token.value as string}</Text>
            ) : (
...
export default function ResolveDuplicateTokenGroup({
  ...
  setName: string;
  group: [string, SingleToken[]];
  onRadioClick: (value: string) => void;
  selectedTokens: { [key: string]: { [key: string]: number | string } };
}) {
  ...
  return (
    ...
    >
      <Stack direction="row" css={{ marginBottom: '$4' }}>
        <Heading size="small">
          <Box as="span" css={{ minWidth: '$9', display: 'inline-flex' }}>
            Token
          </Box>
          {groupKey}
        </Heading>
      ...
      <Stack direction="row">
        <Heading size="small" css={{ minWidth: '$9' }}>
          Value
        </Heading>
        <RadioGroup onValueChange={onRadioClick} value={checkedToken} css={{ gap: '$3' }}>
          {groupValue.map((uniqueToken, i) => (
            // eslint-disable-next-line react/no-array-index-key
            <RadioItem
              key={`${uniqueToken.name}-${i}`}
              value={`${setName}:${uniqueToken.name}:${i}`}
              css={{ alignItems: 'center' }}
            >
              <RadioItemBefore
                data-state={checkedToken === `${setName}:${uniqueToken.name}:${i}` ? 'checked' : 'unchecked'}
                css={{ flexShrink: 0 }}
              >
                <RadioIndicator />
              </RadioItemBefore>
