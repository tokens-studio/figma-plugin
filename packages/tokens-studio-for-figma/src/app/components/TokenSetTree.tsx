...
import { findOrderableTargetIndexesInTokenSetTreeList, tokenSetListToTree, TreeItem } from '@/utils/tokenset';
...
export default function TokenSetTree({
  ...
  React.useEffect(() => {
    ...
    if (
      !isEqual(
        Object.keys(tokens),
        externalItems.filter(({ isLeaf }) => isLeaf).map(({ path }) => path),
      )
    ) {
  ...
  const determineCheckedState = useCallback(
    (item: TreeItem) => {
      ...
      if (childTokenSetStatuses.every((status) => status === TokenSetStatus.ENABLED)) {
        // @README all children are ENABLED
        return true;
      }

      if (
        childTokenSetStatuses.some((status) => status === TokenSetStatus.ENABLED || status === TokenSetStatus.SOURCE)
      ) {
  ...
  const handleReorder = React.useCallback(
    (reorderedItems: ExtendedTreeItem[]) => {
      const nextItems = reorderedItems.reduce<typeof items>((acc, item) => {
        ...
        if (found) {
          ...
          if (itemIsCollapsed && !item.isLeaf) {
            ...
            return acc.concat(
              items.filter((possibleChild) => possibleChild.parent && possibleChild.parent.startsWith(item.path)),
            );
          }
        }
        return acc;
      ...
      debouncedOnReorder(nextItems.filter(({ isLeaf }) => isLeaf).map(({ path }) => path));
    },
  ...
  const handleCheckReorder = React.useCallback(
    (
      ...
      const availableIndexes = findOrderableTargetIndexesInTokenSetTreeList(velocity, value, order);
  ...
  return (
    <ReorderGroup layoutScroll values={mappedItems} checkReorder={handleCheckReorder} onReorder={handleReorder}>
