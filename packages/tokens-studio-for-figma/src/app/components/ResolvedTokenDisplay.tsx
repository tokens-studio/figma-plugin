...
export default function ResolvedTokenDisplay({
  ...
  const valueToCheck = React.useMemo(() => (selectedToken ? selectedToken?.value : alias), [selectedToken, alias]);

  if (selectedToken && isSingleTypographyToken(selectedToken)) {
    return <ResolvedTypographyValueDisplay value={valueToCheck as SingleTypographyToken['value']} />;
  }
  ...
  if (selectedToken && isSingleBorderToken(selectedToken)) {
    return <ResolvedBorderValueDisplay value={valueToCheck as SingleBorderToken['value']} />;
  }
  ...
  return <Box css={{ display: 'flex', flexDirection: 'column', gap: '$2' }}>{valueToCheck}</Box>;
}
