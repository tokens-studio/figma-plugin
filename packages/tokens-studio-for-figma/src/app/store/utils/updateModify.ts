...
export function updateModify(token: SingleToken, data: TokenToRename) {
  ...
  if (
    ...
    token.$extensions?.['studio.tokens'].modify.value
  ) {
    updatedModify = Object.entries(token.$extensions?.['studio.tokens'].modify).reduce<ColorModifier>(
      (modify, [key, value]: string[]) => {
        modify = {
          ...modify,
