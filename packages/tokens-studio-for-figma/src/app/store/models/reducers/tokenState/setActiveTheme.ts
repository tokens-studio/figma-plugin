...
export function setActiveTheme(
  state: TokenState,
  { newActiveTheme }: { newActiveTheme: Record<string, string>; shouldUpdateNodes?: boolean },
): TokenState {
  // Filter activeThemes
  const activeThemeObjectList = state.themes.filter((theme) =>
    Object.values(newActiveTheme).some((v) => v === theme.id),
  );
  ...
  const usedTokenSetsMap =
    activeThemeObjectList.length > 0
      ? Object.fromEntries(
          Object.keys(state.tokens).map((tokenSet) => [
            tokenSet,
            selectedTokenSets?.[tokenSet] ?? TokenSetStatus.DISABLED,
          ]),
        )
      : Object.fromEntries(Object.keys(state.tokens).map((tokenSet) => [tokenSet, TokenSetStatus.DISABLED]));
  return {
    ...state,
