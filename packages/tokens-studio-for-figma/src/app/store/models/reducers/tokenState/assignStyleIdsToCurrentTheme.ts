...
export function assignStyleIdsToCurrentTheme(
  state: TokenState,
  {
    styleIds,
    tokens,
    selectedThemes,
  }: { styleIds: Record<string, string>; tokens: ResolveTokenValuesResult[]; selectedThemes: string[] },
): TokenState {
  ...
  Object.entries(styleIds).forEach(([tokenName, styleId]) => {
    ...
    const activeTheme = activeThemes.find((theme) => Object.entries(theme.selectedTokenSets).some(
      ([tokenSet, status]) => status === TokenSetStatus.ENABLED && tokenSet === tokens.find((t) => t.name === tokenName)?.internal__Parent,
    ));
    ...
    if (themeObjectIndex !== -1) {
      updatedThemes.splice(themeObjectIndex, 1, {
        ...
        $figmaStyleReferences: {
          ...updatedThemes[themeObjectIndex].$figmaStyleReferences,
          ...{ [tokenName]: styleId },
        },
      });
    }
