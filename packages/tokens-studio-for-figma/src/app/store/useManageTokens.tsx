...
export default function useManageTokens() {
  ...
  const editSingleToken = useCallback(
    async (data: EditSingleTokenData) => {
      const { parent, type, name, value, description, oldName, shouldUpdateDocument = true, $extensions } = data;
  ...
  const createSingleToken = useCallback(
    async (data: CreateSingleTokenData) => {
      const { parent, type, name, value, description, shouldUpdateDocument = true, $extensions } = data;
      dispatch.uiState.startJob({
  ...
  const duplicateSingleToken = useCallback(
    async (data: DuplicateTokenPayload) => {
      dispatch.uiState.startJob({
  ...
  const deleteSingleToken = useCallback(
    async (data: DeleteTokenPayload) => {
      const choices: Choice[] = [];
      if (
        store.getState().tokenState.themes.length > 0 &&
        data.type &&
        [TokenTypes.COLOR, TokenTypes.TYPOGRAPHY, TokenTypes.BOX_SHADOW].includes(data?.type)
      ) {
        choices.push({
          key: StyleOptions.REMOVE,
          label: 'Delete associated style',
        });
      }
  ...
  const deleteDuplicates = useCallback(
    async (duplicates: { set: string; path: string; index: number }[]) => {
      const userConfirmation = await confirm({
  ...
  const renameGroup = useCallback(
    async (oldName: string, newName: string, type: string): Promise<TokenToRename[]> => {
      const activeTokenSet = activeTokenSetSelector(store.getState());
  ...
  const duplicateGroup = useCallback(
    (data: Omit<DuplicateTokenGroupPayload, 'parent'>) => {
      const activeTokenSet = activeTokenSetSelector(store.getState());
  ...
  const renameTokensAcrossSets = useCallback(
    async (oldName: string, newName: string, type: string, tokenSets: string[]) => {
  ...
  const importMultipleTokens = useCallback(
    async ({
      multipleUpdatedTokens,
      multipleNewTokens,
    }: {
      multipleUpdatedTokens?: EditSingleTokenData[];
      multipleNewTokens?: CreateSingleTokenData[];
    }) => {
