...
describe('setupReplay', () => {
  ...
  it('does not throw when client is not available', () => {
    jest.useFakeTimers();
    const getClient = jest.fn().mockReturnValue(undefined);
  ...
  it('handles errors from getCurrentHub gracefully', () => {
    jest.useFakeTimers();
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Error setting up Sentry Replay:', expect.any(Error));
  });
});
