import { checkReorder } from '../motion/checkReorder';
...
describe('checkReorder', () => {
  ...
  test('Takes into account available indexes', () => {
    const newOrder = checkReorder(order, 'a', 100, 1, [0, 2]);
    expect(newOrder).not.toEqual(order);
    expect(newOrder[0].value).toBe('b');
    expect(newOrder[1].value).toBe('c');
    expect(newOrder[2].value).toBe('a');
  });
});
