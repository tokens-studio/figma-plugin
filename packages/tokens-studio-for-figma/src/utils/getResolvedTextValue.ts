  ...
  if (token.type === TokenTypes.TYPOGRAPHY || token.type === TokenTypes.BOX_SHADOW) {
    if (Array.isArray(token.value)) {
      const array = token.value.reduce<Array<string>>((totalAcc, item) => {
        const singleReturnValue = Object.entries(item).reduce<Array<string>>(
          (acc, [, propertyValue]) => acc.concat(`${propertyValue.toString()}`),
          [],
        );
      ...
      const array = Object.entries(token.value).reduce<Array<string>>(
        (acc, [, propertyValue]) => acc.concat(`${propertyValue.toString()}`),
        [],
      );
      returnValue = array.join('/');
    ...
    const array = Object.entries(token.value).reduce<Array<string>>(
      (acc, [property, value]) => acc.concat(`${property}:${value}`),
      [],
    );
    returnValue = array.join(',');
