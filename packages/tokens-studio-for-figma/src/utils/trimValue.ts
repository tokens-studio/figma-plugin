...
export default function trimValue(value: SingleToken['value'], tokenType?: TokenTypes): SingleToken['value'] {
  ...
  if (Array.isArray(value)) {
    return value.map((item) => {
      ...
      // wiping the value. Only reduce object elements; pass primitives
      // through with a trim on strings and no-op on numbers/null.
      if (item == null || typeof item !== 'object') {
        return typeof item === 'string' ? item.trim() : item;
      }
      return Object.entries(item).reduce<Record<string, string>>((acc, [key, val]) => {
