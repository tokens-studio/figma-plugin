...
export function isSingleTypographyValue(
  value: MapValuesToTokensResult[string],
): value is SingleTypographyToken['value'] {
  return Boolean(
    value
      && (typeof value === 'string'
        || (
          (Array.isArray(value) ? value : [value]) as (
            ...
            | Extract<MapValuesToTokensResult[string], { property: string }[]>[number]
          )[]
        ).every(
          (v) => v
            && typeof v === 'object'
