...
export default function checkIfTokenCanCreateVariable(
  ...
  if (
    ...
    if (
      (token.type === TokenTypes.BORDER_RADIUS || token.type === TokenTypes.SPACING)
    ...
    if (
      ...
      && (token.value.startsWith('linear-gradient')
        || token.value.startsWith('radial-gradient')
        || token.value.startsWith('conic-gradient'))
    ) {
      return false;
    }
    ...
    if (
      ...
      && token.type !== TokenTypes.OPACITY
      && typeof token.value === 'string'
      && numberMatchesPercentage(token.value)
    ) {
      return false;
    }
    return true;
