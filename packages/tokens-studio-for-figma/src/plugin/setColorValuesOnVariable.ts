...
export default function setColorValuesOnVariable(variable: Variable, mode: string, value: string) {
  try {
    ...
    const existingValue = isFigmaColorObject(existingVariableValue)
      ? normalizeFigmaColor(existingVariableValue)
      : existingVariableValue;
    const newValue = isFigmaColorObject(color)
      ? normalizeFigmaColor({ ...color, a: opacity })
      : { ...color, a: opacity };
