...
export async function applyOpacityValuesOnNode(node: BaseNode, data: NodeTokenRefMap, values: MapValuesToTokensResult, baseFontSize: string) {
  ...
  if (
    ...
    if (!(await tryApplyVariableId(node, 'opacity', data.opacity))) {
