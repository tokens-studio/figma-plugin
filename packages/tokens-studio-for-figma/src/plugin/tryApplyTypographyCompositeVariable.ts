...
export async function tryApplyTypographyCompositeVariable({
  ...
  try {
    ...
    for (const [originalKey, val] of Object.entries(resolvedValue).filter(([_, keyValue]) => typeof keyValue !== 'undefined')) {
      ...
      if (value[originalKey].toString().startsWith('{') && value[originalKey].toString().endsWith('}') && shouldCreateStylesWithVariables) {
        const variableToApply = await defaultTokenValueRetriever.getVariableReference(value[originalKey].toString().slice(1, -1));
