...
export default async function pullVariables(
  ...
  if (options.useRem) {
    ...
    if (settings?.baseFontSize) {
      const baseFontSizeValue = typeof settings.baseFontSize === 'number' ? settings.baseFontSize : parseFloat(settings.baseFontSize);
      ...
      if (existingSet.includes('/') && !currentTokenSets.has(existingSet)) {
        ...
        const matchingTheme = themeInfo.themes?.find((t) => {
          ...
          const hasMatchingCurrentTheme = themesToCreate.some(
            (currentTheme) => currentTheme.$figmaCollectionId === t.$figmaCollectionId && currentTheme.$figmaModeId === t.$figmaModeId,
          );
        ...
        if (matchingTheme) {
          const currentTheme = themesToCreate.find(
            (t) => t.$figmaCollectionId === matchingTheme.$figmaCollectionId
