...
export default function EditStorageItemForm({handleChange, handleSubmit, handleCancel, values, hasErrored}) {
    React.useEffect(() => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Input
                full
                label={`ID${values.provider === StorageProviderType.JSONBIN ? ' (optional)' : ''}`}
                value={values.id}
                onChange={handleChange}
                type="text"
                name="id"
                required={values.provider !== StorageProviderType.JSONBIN}
            />
