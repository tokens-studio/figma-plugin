...
import { activeTokenSetSelector, displayTypeSelector, tokenGroupDescriptionSelector } from '@/selectors';
...
const TokenGroup: React.FC<Props> = ({
  ...
  const tokenGroupDescription = useSelector(tokenGroupDescriptionSelector);
        ...
        <React.Fragment key={item.stringPath}>
          ...
          // Need to add class to self-reference in css traversal
            <StyledTokenGroupItems className="property-wrapper" data-cy={`token-group-${item.stringPath}`}>
              <TokenGroupHeading showNewForm={showNewForm} label={item.name} path={item.stringPath} id="listing" type={schema.type} description={tokenGroupDescription?.[activeTokenSet]?.[schema.type]?.[item.stringPath]} />
              <TokenGroup
