...
export default function useManageTokens() {
  ...
  const duplicateGroup = useCallback(
    async (data: { newPath: string; oldName: string }) => {
      ...
      dispatch.uiState.completeJob(BackgroundJobs.UI_DUPLICATTOKENGROUP);
    },
    [store, duplicateTokenGroup, dispatch.uiState]
  );

  const renameGroup = useCallback(
    async (data: { path: string; oldName: string; newName: string }) => {
      const activeTokenSet = activeTokenSetSelector(store.getState());
