...
export function useSupernova() {
  ...
  const pushToSupernova = useCallback(
    async (context: SupernovaCredentials): Promise<RemoteTokenStorageData<SupernovaStorageMetadata> | null> => {
    ...
    [activeTheme, dispatch.tokenState, dispatch.uiState, pushDialog, storageClientFactory, themes, tokens, usedTokenSet],
  );

  const pullFromSupernova = useCallback(
    async (context: SupernovaCredentials) => {
      console.log('PULL FROM SUPERNOVA');
      const storage = storageClientFactory(context);
    ...
    [dispatch.tokenState, storageClientFactory],
  );
  ...
  const syncTokensWithSupernova = useCallback(
    ...
    [askUserIfPull, dispatch, pushToSupernova, storageClientFactory, usedTokenSet, activeTheme, themes, tokens]
  );

  const addNewSupernovaCredentials = useCallback(
    async (context: SupernovaFormValues): Promise<RemoteTokenStorageData<SupernovaStorageMetadata> | null> => {
