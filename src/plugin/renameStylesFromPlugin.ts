...
export default async function renameStylesFromPlugin(
  ...
  tokensToRename.forEach((token) => {
    oldPathNames = oldPathNames.concat(themesToContainToken.map((theme) => convertTokenNameToPath(token.oldName, theme)).concat(themesToContainToken.map(() => convertTokenNameToPath(token.oldName, null, 1))).concat(convertTokenNameToPath(token.oldName)));
    newPathNames = newPathNames.concat(themesToContainToken.map((theme) => convertTokenNameToPath(token.newName, theme)).concat(themesToContainToken.map(() => convertTokenNameToPath(token.newName, null, 1))).concat(convertTokenNameToPath(token.newName)));
  });
