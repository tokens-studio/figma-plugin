...
export class BitbucketTokenStorage extends GitTokenStorage {
  ...
  public async createBranch(branch: string, source?: string) {
    try {
      ...
      const originBranch = await this.bitbucketClient.git.getRef({
        owner: this.owner,
  ...
  public async canWrite(): Promise<boolean> {
    const currentUser = await this.bitbucketClient.rest.users.getAuthenticated();
    if (!currentUser.data.login) return false;
    try {
      const canWrite = await this.bitbucketClient.rest.repos.getCollaboratorPermissionLevel({
        owner: this.owner,
        repo: this.repository,
  ...
  public async read(): Promise<RemoteTokenStorageFile<GitStorageMetadata>[]> {
    try {
      const response = await this.bitbucketClient.rest.repos.getContent({
        owner: this.owner,
        repo: this.repository,
        path: this.path,
        ref: this.branch,
